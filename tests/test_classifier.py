...
def test_classifier():
    ...
    df = classifier(train_df, "income", numeric_feats=['age', 'fnlwgt', 'hours.per.week', 'education.num', 'capital.gain', 'capital.loss'], categorical_feats=['occupation'])
    ...
    assert isinstance(df, pd.DataFrame), "Not valid type"
    ...
    def test_classifier_target() :
        ...
        with pytest.raises(TypeError) as e:
            classifier(1, target_col = "income", numeric_feats=['age'], categorical_feats = ['occupation'])
        assert str(e.value) == "Invalid function input. Please enter a data frame"

        with pytest.raises(TypeError) as e:
            classifier(income_df, target_col = "income", numeric_feats=['age'], categorical_feats = 1)
        assert str(e.value) == "Categorical Features should be passed as a list"

        with pytest.raises(TypeError) as e:
            classifier(income_df, target_col = 1, numeric_feats=['age'], categorical_feats = ['occupation'])
        assert str(e.value) == "Target column must be passed as a string"
